...
function clearPropertySelection() {
  ...
  // Clear the content of the detail divs
  const propertyDetailsTableBody = document.querySelector('#property-details tbody');
  if (propertyDetailsTableBody) {
    propertyDetailsTableBody.innerHTML = ''; // Clear table content
  }
...
export async function showProperties(cik, title) {
  try {
    ...
    if (properties && properties.length > 0) {
      ...
      propertyContainer.innerHTML = '<h5>Property Data</h5>';
      propertyContainer.innerHTML += '<div id="map"></div>';
      propertyContainer.innerHTML += '<div id="location-map"></div>';
      propertyContainer.innerHTML += '<div id="location-pano"></div>';
      propertyContainer.innerHTML += '<table id="property-details" border="1" style="width: 100%; margin-top: 20px;"><tbody></tbody></table>';
      propertyContainer.innerHTML += '<button id="clear-selection-btn" class="btn waves-effect waves-light blue darken-1" style="margin-top: 10px; margin-bottom: 20px;"><i class="material-icons left">clear</i>Clear Property Selection</button>';
      ...
      properties.forEach(property => {
        ...
        marker.addListener('click', async () => {
           ...
           // If the clicked marker is already the selected one, deselect it
          ...
          setLocationVisible(); // Show the location map and Street View
          ...
          const tableBody = document.querySelector('#property-details tbody');
          ...
          try {
            ...
            if (geocodeData.status === "OK" && geocodeData.results.length > 0) {
              const originalLat = geocodeData.results[0].geometry.location.lat;
              const originalLng = geocodeData.results[0].geometry.location.lng;
              const originalLocation = { lat: parseFloat(originalLat), lng: parseFloat(originalLng) };

              console.log(`Geocoded ${address} to original lat: ${originalLat}, lng: ${originalLng}`);
              ...
              streetViewService.getPanorama({
                ...
                if (status === google.maps.StreetViewStatus.OK && data.location) {
                  ...
                  console.log(`Found Street View panorama at lat: ${panoLocation.lat()}, lng: ${panoLocation.lng()}`);
                  ...
                  const panorama = new google.maps.StreetViewPanorama(
                    document.getElementById("location-pano"),
                    {
                  ...
                  console.warn(`No Street View data available for ${address} within ${radius}m radius. Status: ${status}`);
              ...
              console.error(`Geocoding failed for ${address}. Status: ${geocodeData.status}`);
                ...
                <li class="active">
                  ...
                  <div class="collapsible-body">
                    ...
                    <p><strong>Largest Tenant:</strong> ${property.largest_tenant || 'N/A'}
                      ...
                      lease expires ${extractDate(property.lease_expiration_largest_tenant_date)} </p>
                    <p><strong>Second Largest Tenant:</strong> ${property.second_largest_tenant || 'N/A'}
                      (${formatNumber(property.square_feet_second_largest_tenant)} Sq Ft)
                      lease expires ${extractDate(property.lease_expiration_second_largest_tenant_date)} </p>
                    <p><strong>Third Largest Tenant:</strong> ${property.third_largest_tenant || 'N/A'}
                      (${formatNumber(property.square_feet_third_largest_tenant)} Sq Ft)
